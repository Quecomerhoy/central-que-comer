...
const wait=()=>{
  if(typeof QC==='undefined'||!Array.isArray(QC.cart)||!('orderForPeople' in QC)){setTimeout(wait,80);return}
  init();
};
function init(){
  ...
  bindButton();setTimeout(bindButton,100);setTimeout(bindButton,500);
  if(typeof renderCart==='function'){const baseRender11827=renderCart;renderCart=function(){const r=baseRender11827.apply(this,arguments);updateButton();if(document.getElementById('assembleOverlay')?.classList.contains('show'))renderAssembler();return r}}
  updateButton();
}
wait();
})();
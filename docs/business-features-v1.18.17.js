...
const qcBaseChangeStatus1193=changeStatus;
changeStatus=async function(id,status){
  const order=A.orders.find(x=>String(x.id)===String(id));
  const current=String(order?.status||'nuevo');
  if(String(status)===current){renderOrders();return}
  const messages={
    nuevo:'¿Confirmas regresar este pedido a NUEVO?',
    preparando:'¿Confirmas cambiar este pedido a PREPARANDO? El cliente verá que ya se está atendiendo.',
    listo:'¿Confirmas marcar este pedido como LISTO?',
    cancelado:'¿Confirmas CANCELAR este pedido? El pedido pasará al historial y, si usó saldo, se devolverá al cliente.'
  };
  if(!confirm(messages[status]||('¿Confirmas cambiar el estado a '+String(status).toUpperCase()+'?'))){
    renderOrders();
    toast('Cambio de estado cancelado');
    return;
  }
  return qcBaseChangeStatus1193.call(this,id,status)
};

// Sincronización efectiva del % de uso definido por Control Central.
// Si una sucursal tiene comisión individual, conserva esa tarifa; de lo contrario usa la general.
function qcBusinessHasCustomCommission1192(b){return !!(b&&(b.commissionSource==='branch'||b.commissionMode==='custom'))}
async function qcSyncUsageFee1192(){
  if(!A.dbUrl||A.accessStatus!=='active'||!A.businessId)return;
  try{
    const [pc,b]=await Promise.all([get('quecomer/platform/config').catch(()=>null),get('quecomer/businesses/'+A.businessId).catch(()=>null)]);
    if(!b)return;
    const general=pc&&Number.isFinite(Number(pc.defaultFee))?Number(pc.defaultFee):Number(A.defaultFee);
    const branch=Number(b.commissionPct),custom=qcBusinessHasCustomCommission1192(b);
    const effective=custom&&Number.isFinite(branch)?branch:general;
    if(!Number.isFinite(effective))return;
    if(!custom&&Number.isFinite(general)&&Math.abs(branch-general)>0.000001){
      const now=new Date().toISOString();
      await patch('quecomer/businesses/'+A.businessId,{commissionPct:general,commissionSource:'default',commissionMode:'default',commissionUpdatedAt:now,commissionUpdatedBy:'Configuración central',updatedAt:now}).catch(()=>{});
      const pu={};(A.products||[]).forEach(p=>{if(p?.id){pu[p.id+'/commissionPct']=general;pu[p.id+'/updatedAt']=now}});if(Object.keys(pu).length)await patch('quecomer/products',pu).catch(()=>{});
    }
    const before=Number(A.defaultFee);
    A.defaultFee=effective;
    localStorage.setItem('qc_platform_fee_v08',effective);localStorage.setItem('qc_platform_fee',effective);
    if(Math.abs(effective-before)>0.000001){
      (A.products||[]).forEach(p=>p.commissionPct=effective);
      if(typeof renderProducts==='function')renderProducts();
      const feeLabel=document.getElementById('productBranchFee');if(feeLabel)feeLabel.textContent=effective.toFixed(2)+'% · definida por Control central';
      toast('Uso de app actualizado a '+effective.toFixed(2)+'%');
    }
  }catch(e){}
}
qcSyncUsageFee1192();
setInterval(qcSyncUsageFee1192,5000);
})();